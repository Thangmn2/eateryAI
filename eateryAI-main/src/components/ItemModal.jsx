    ...
    >
      ...
      >
        ...
        <div className="p-5 sm:p-6">
          ...
          {/* Restaurant & Category */}
          <div className="flex items-center gap-2 text-sm text-warmgray mb-3">
          ...
          {(isEstimated || isScanned) && (
            <div className="flex items-start gap-2 mb-3 px-3 py-2.5 rounded-xl bg-amber-50 border border-amber-200/60">
          ...
          {nutrition.length > 0 && (
            <div className="flex gap-3 mb-5">
              {nutrition.map(n => (
                <div key={n.label} className={`flex-1 rounded-xl px-3 py-2.5 text-center ${isEstimated ? 'bg-amber-50/50 border border-dashed border-amber-200/50' : 'bg-ivory'}`}>
                  <div className="flex items-center justify-center gap-1.5 mb-0.5">
                    <span className={`w-2 h-2 rounded-full ${n.color}`} />
                    <span className="text-[11px] uppercase tracking-wider text-warmgray font-medium">
                      {isEstimated ? '~' : ''}{n.label}
                    </span>
                  </div>
          ...
          {/* Address */}
          {item.Address && (
            <div className="flex items-start gap-2 text-xs text-warmgray-light mb-5 bg-ivory/50 rounded-lg px-3 py-2">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center bg-ivory rounded-xl overflow-hidden border border-cream">
              <button
            ...
            <button
              ...
              className="flex-1 h-12 rounded-xl bg-terra text-white font-semibold text-sm
                         hover:bg-terra-dark active:scale-[0.98] transition-all shadow-sm
